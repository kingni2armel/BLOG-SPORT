                           ...
                           //requette qui permet de selectionner toute les categories de la base de donnee //
                            ...
                            if (isset($_POST['description']) && isset($_POST['selectCategorie']) && isset($_POST['dates'])) {
                                ...
                                if (!empty($_POST['description']) && !empty($_POST['selectCategorie']) && !empty($_POST['dates'])) {
                                    ...
                                    if(!empty($_FILES)){
                                            // declaration de la  variable qui va possseder le  nom de l'image
                                        $file_name= $_FILES['fichier']['name'];
                                        //declaration de la variable qui va contenir l'extension de l'image
                                        $file_extension= strrchr($file_name,".");
                                        // declaration de la variable  qui va contenir le tmp de l'image
                                        $file_tmp_name= $_FILES['fichier']['tmp_name'];
                                        // declaration de la variable qui va contenir le lien vers le repertoire qui va contenir les images
                                        $file_dest ='files/'.$file_name;
                                        //declaration de la variable qui va contenir le tableau qui prend en parametre toutes les extensions qui sont autorises
                                        $extension_autorise = array('.png','.PNG','.jpeg','.JPEG','.jpg','.JPG');
                                        if(in_array($file_extension,$extension_autorise)) 
                                        {
                                                if(move_uploaded_file($file_tmp_name,$file_dest)){
                                                    //envoie des donnees dans la table image
                                                    $reqs=$bdd->prepare('INSERT INTO image(name,file_url) values (?,?)');
                                                    $description = $_POST['description'];
                                                    $id_categorie = $_POST['selectCategorie'];
                                                    $date = $_POST['dates']; 
                                                    //envoie des donnees dans la table publication
